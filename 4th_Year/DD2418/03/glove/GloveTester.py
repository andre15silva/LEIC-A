...
class GloveTester:
    ...
    def find_nearest(self, words, metric='cosine'):
        ...
        W = list(self.w_vector.values())
        nbrs = NearestNeighbors(n_neighbors=5, metric=metric).fit(W)

        # Get vectors of the query data
        assert(all([x in self.word2id for x in words]))
        query = np.array(list(map(lambda x: W[x], [self.word2id[x] for x in words])))
        ...
        for i in range(len(words)):
            _result = list(zip(list(map(lambda x: self.id2word[x], indices[i])), distances[i]))
            result += [_result]
