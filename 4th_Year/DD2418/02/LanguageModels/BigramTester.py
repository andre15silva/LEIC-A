...
class BigramTester(object):
    ...
    def compute_entropy_cumulatively(self, word):
        ...
        if self.last_index >= 0:
            ...
            if word in self.bigram_prob[last_word]:
                cond_prob = math.exp(self.bigram_prob[last_word][word])

        wprob = 0
        if word in self.unigram_count:
            wprob = self.unigram_count[word] / self.total_words
            self.last_index = self.index[word]
        else:
            self.last_index = -1

        prob = self.lambda1 * cond_prob + self.lambda2 * wprob + self.lambda3

        self.logProb -= math.log(prob) / len(self.tokens)
