...
class RestrictedBoltzmannMachine():
    ...
    def cd1_for_rbm(self, visible_trainset, n_iterations=10000, avg_recon_loss=False):
        x = []
        recon_loss = []
        # k_iters = 1
        """Contrastive Divergence with k=1 full alternating Gibbs sampling
        ...
        for it in range(n_iterations):
            ...
            if it % self.print_period == 0:
                ...
                if avg_recon_loss:
                    ...
                    it, np.mean(np.abs(mini_batches[idx] - reconstruction_batch))))
                # print ("iteration=%7d recon_loss=%4.4f"%(it, np.linalg.norm(mini_batches[idx] - reconstruction_batch)))
    ...
    def get_h_given_v_dir(self,visible_minibatch):
        ...
        probabilities =  sigmoid(visible_minibatch @ self.weight_v_to_h + self.bias_v)
        activations = sample_binary(probabilities)
        return probabilities, activations
