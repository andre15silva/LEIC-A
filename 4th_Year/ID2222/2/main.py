...
def main():
    with open("./T10I4D100K.dat", "r") as fp:
        buckets = []
        for line in fp:
            buckets += [set([int(a) for a in line.rstrip("\n").split()])]
    fi = FrequentItemsets(buckets)
    print(fi.frequentItemsets)
    ar = AssociationRules(buckets, 0.5, fi.frequentItemsets)
    # ar = AssociationRules(buckets, 0.5, fItems)
    print(ar)
